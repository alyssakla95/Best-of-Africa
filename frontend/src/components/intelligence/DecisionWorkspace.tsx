...
export function DecisionWorkspace({ context }: DecisionWorkspaceProps) {
  ...
  return <section className="page-section space-y-8" aria-labelledby={`${context}-decision-workspace`}>
    ...
    <div className="grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
      <section className="rounded-2xl border border-border bg-white p-5 md:p-8"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[.16em] text-navy/60">Macroeconomic record</p><h3 className="mt-2 font-serif text-3xl text-navy">Observed country indicators</h3></div><a href={officialProfile!.source_url} target="_blank" rel="noopener noreferrer" className="text-xs font-semibold text-navy underline underline-offset-4">Inspect official provider <ExternalLink size={12} className="inline"/></a></div><div className="mt-6 grid gap-3 sm:grid-cols-2">{macroIndicators.map(indicator => <article key={indicator.code} className="rounded-xl border border-border p-4"><p className="text-[9px] font-bold uppercase tracking-[.1em] text-muted-foreground">{indicator.code} · {indicator.year}</p><h4 className="mt-2 text-sm font-bold text-navy">{text(indicator.name)}</h4><p className="mt-3 font-serif text-2xl text-navy">{evidenceValue(indicator.value,indicator.unit)}</p><p className="mt-2 text-[10px] font-semibold uppercase tracking-[.08em] text-navy/55">{(indicator as { period_status?: string }).period_status === 'estimate_or_projection' ? 'Estimate or projection' : 'Historical observation'}</p></article>)}</div></section>
