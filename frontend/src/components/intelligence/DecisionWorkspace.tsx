...
export function DecisionWorkspace({ context }: DecisionWorkspaceProps) {
  ...
  return <section className="page-section space-y-8" aria-labelledby={`${context}-decision-workspace`}>
    ...
    <div className="grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
      <section className="rounded-2xl border border-border bg-white p-5 md:p-8"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[.16em] text-navy/60">Macroeconomic record</p><h3 className="mt-2 font-serif text-3xl text-navy">{macroIndicators.length} observed country indicators</h3><p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">Official observations are organised by the decision question they help answer. Every measure retains its year, unit, series code and direct provider link.</p></div><a href={officialProfile!.source_url} target="_blank" rel="noopener noreferrer" className="text-xs font-semibold text-navy underline underline-offset-4">Inspect complete provider record <ExternalLink size={12} className="inline"/></a></div><div className="mt-7 space-y-8">{groupedMacroIndicators.map(group => <section key={group.category}><div className="flex items-center justify-between gap-3 border-b border-border pb-3"><h4 className="text-sm font-bold text-navy">{text(group.category)}</h4><span className="text-[10px] font-semibold text-muted-foreground">{group.indicators.length} measures</span></div><div className="mt-3 grid gap-3 sm:grid-cols-2">{group.indicators.map(indicator => <article key={indicator.code} className="flex min-w-0 flex-col rounded-xl border border-border p-4"><p className="text-[9px] font-bold uppercase tracking-[.1em] text-muted-foreground">{indicator.code} · {indicator.year}</p><h5 className="mt-2 text-sm font-bold text-navy">{text(indicator.name)}</h5><p className="mt-3 break-words font-serif text-2xl text-navy">{evidenceValue(indicator.value,indicator.unit)}</p>{indicator.decision_use && <p className="mt-3 text-xs leading-5 text-navy/70">{text(indicator.decision_use)}</p>}<div className="mt-auto flex flex-wrap items-center justify-between gap-2 pt-4"><span className="text-[9px] font-semibold uppercase tracking-[.08em] text-navy/55">{indicator.period_status === 'estimate_or_projection' ? 'Estimate or projection' : 'Historical observation'}</span><a href={indicator.source_url} target="_blank" rel="noopener noreferrer" className="text-[10px] font-semibold text-navy underline underline-offset-4">Series source <ExternalLink size={10} className="inline"/></a></div></article>)}</div></section>)}</div></section>
