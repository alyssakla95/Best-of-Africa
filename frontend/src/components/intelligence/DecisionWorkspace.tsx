...
export function DecisionWorkspace({ context }: DecisionWorkspaceProps) {
  ...
  return <section className="page-section space-y-8" aria-labelledby={`${context}-decision-workspace`}>
    ...
    <section className="overflow-hidden rounded-2xl border border-border bg-white"><div className="border-b border-border px-5 py-6 md:px-8"><p className="text-[10px] font-bold uppercase tracking-[.16em] text-navy/60">Decision readiness register</p><h3 className="mt-2 font-serif text-3xl text-navy">What is evidenced now, and what must be verified next</h3></div><div className="grid gap-3 p-4 md:hidden">{decisionRows.map(([area,status,evidence,next]) => <details key={area} className="overflow-hidden rounded-xl border border-border bg-white"><summary className="cursor-pointer px-4 py-4 marker:text-navy"><h4 className="inline font-bold text-navy">{area}</h4><p className="mt-1 text-[10px] font-bold uppercase tracking-[.1em] text-navy/55">{status}</p></summary><div className="border-t border-border px-4 py-4"><p className="text-sm leading-6 text-navy/80">{evidence}</p><p className="mt-3 text-sm leading-6 text-muted-foreground"><strong className="text-navy">Next verification:</strong> {next}</p></div></details>)}</div><div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[960px] border-collapse text-left text-sm"><thead className="bg-navy text-white"><tr>{['Decision area','Evidence state','Current record','Next verification'].map(label => <th key={label} className="px-5 py-3 text-[10px] uppercase tracking-[.1em]">{label}</th>)}</tr></thead><tbody className="divide-y divide-border">{decisionRows.map(([area,status,evidence,next]) => <tr key={area} className="align-top"><th className="px-5 py-4 font-semibold text-navy">{area}</th><td className="px-5 py-4 text-xs font-semibold text-navy/70">{status}</td><td className="max-w-sm px-5 py-4 leading-6 text-navy/80">{evidence}</td><td className="max-w-sm px-5 py-4 leading-6 text-muted-foreground">{next}</td></tr>)}</tbody></table></div></section>
    ...
    <div className="grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
      <section className="rounded-2xl border border-border bg-white p-5 md:p-8">
        ...
        <div className="mt-7 space-y-3">{groupedMacroIndicators.map((group,index) => <details key={group.category} open={index === 0} className="overflow-hidden rounded-xl border border-border"><summary className="flex cursor-pointer items-center justify-between gap-3 bg-navy/[.025] px-4 py-3 marker:text-navy"><h4 className="text-sm font-bold text-navy">{text(group.category)}</h4><span className="text-[10px] font-semibold text-muted-foreground">{group.indicators.length} measures</span></summary><div className="grid items-start gap-3 border-t border-border p-3 sm:grid-cols-2">{group.indicators.map(indicator => {
