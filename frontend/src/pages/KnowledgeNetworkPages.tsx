...
export function KnowledgeNetworkSection({ surface, compact = false }: { surface: Surface; compact?: boolean }) {
  ...
  return (
    <div>
      <div className="flex gap-2 overflow-x-auto pb-3 [scrollbar-width:none]"><Button variant={groupType === 'all' ? 'default' : 'outline'} onClick={() => setGroupType('all')}>All groups</Button>{types.map(type => <Button key={type} variant={groupType === type ? 'default' : 'outline'} onClick={() => setGroupType(type)}>{GROUP_LABELS[type]}</Button>)}</div>
