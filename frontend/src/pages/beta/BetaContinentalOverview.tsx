...
export const BetaContinentalOverview: React.FC = () => {
  ...
  if (query.isError || !query.data) return <><SEO title="Continental data unavailable | BOA-Story" description="The continental economic record could not be loaded."/><div className="mx-auto flex min-h-[60vh] max-w-2xl flex-col justify-center px-5 sm:px-6"><p className="text-xs font-bold uppercase tracking-[.2em] text-navy/60">Official dataset request failed</p><h1 className="mt-3 font-serif text-4xl text-navy">The continental economic record could not be loaded.</h1><p className="mt-4 leading-7 text-muted-foreground">Retry the official-data dashboard or continue to individual country dossiers.</p><div className="mt-8 flex flex-wrap gap-3"><button onClick={() => query.refetch()} className="rounded-md bg-navy px-5 py-3 text-sm font-semibold text-white">Retry dashboard</button><Link to="/countries" className="rounded-md border border-border bg-white px-5 py-3 text-sm font-semibold text-navy">Country dossiers</Link></div></div></>;
        ...
        {view === 'overview' && <>
          ...
          <section className="page-section overflow-hidden rounded-2xl border border-border bg-white" aria-labelledby="continental-briefing-scope">
            ...
            <div className="grid gap-4 px-5 py-5 md:grid-cols-2 md:px-8">
              <details className="rounded-xl border border-border"><summary className="cursor-pointer px-4 py-3 text-sm font-semibold text-navy">Inspect all country evidence states</summary><div className="grid max-h-[30rem] gap-2 overflow-y-auto border-t border-border p-3 sm:grid-cols-2">{data.briefing_scope.countries.map(country => <Link key={country.country_code} to={`/countries/${country.country_code}`} className="flex items-center justify-between gap-3 rounded-lg bg-navy/[.035] px-3 py-2 text-xs"><span><strong className="text-navy">{countryName(country.country_code, country.country_name)}</strong><small className="ml-1 text-muted-foreground">{country.region}</small></span><span className="tabular-nums text-muted-foreground">{country.records_30d}</span></Link>)}</div></details>
            ...
            <div className="border-t border-border px-5 py-5 md:px-8">
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <div className="rounded-xl bg-navy/[.035] p-4"><p className="text-[9px] font-bold uppercase tracking-[.1em] text-muted-foreground">Official country acquisition</p><p className="mt-2 font-serif text-2xl text-navy">{data.source_network.countries_with_official_lanes} / {data.briefing_scope.countries_considered}</p><p className="mt-1 text-[10px] text-muted-foreground">countries with a direct provider lane</p></div>
                <div className="rounded-xl bg-navy/[.035] p-4"><p className="text-[9px] font-bold uppercase tracking-[.1em] text-muted-foreground">Country lanes productive</p><p className="mt-2 font-serif text-2xl text-navy">{data.source_network.official_country_lanes_productive_30d}</p><p className="mt-1 text-[10px] text-muted-foreground">supplied qualifying evidence in 30 days</p></div>
                <div className="rounded-xl bg-navy/[.035] p-4"><p className="text-[9px] font-bold uppercase tracking-[.1em] text-muted-foreground">Direct sources productive</p><p className="mt-2 font-serif text-2xl text-navy">{data.source_network.productive_direct_sources_30d} / {data.source_network.active_direct_sources}</p><p className="mt-1 text-[10px] text-muted-foreground">measured output, not catalogue size</p></div>
                <div className="rounded-xl bg-navy/[.035] p-4"><p className="text-[9px] font-bold uppercase tracking-[.1em] text-muted-foreground">Primary/global productive</p><p className="mt-2 font-serif text-2xl text-navy">{data.source_network.productive_primary_or_global_sources_30d} / {data.source_network.active_primary_or_global_sources}</p><p className="mt-1 text-[10px] text-muted-foreground">authoritative source lanes</p></div>
              </div>
              <p className="mt-4 text-xs leading-5 text-muted-foreground">{data.source_network.methodology}</p>
            </div>
