          ...
          {coverageQuery.data && <section className="page-section overflow-hidden rounded-2xl border border-border bg-white" aria-labelledby="all-market-scope">
            ...
            <div className="border-t border-border bg-navy/[.025] px-5 py-7 md:px-8">
              <div className="grid gap-6 lg:grid-cols-[.75fr_1.25fr] lg:items-start">
                <div><p className="text-[10px] font-bold uppercase tracking-[.16em] text-navy/60">Source breadth and concentration</p><h3 className="mt-2 font-serif text-2xl text-navy">Who supplies the rolling evidence window</h3><p className="mt-3 text-xs leading-5 text-muted-foreground">Publisher breadth, authority and concentration are shown separately from story volume. The concentration readings make an imbalanced evidence window visible instead of allowing a long source list to conceal it.</p><dl className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-xl bg-white p-4 ring-1 ring-border"><dt className="text-[9px] font-bold uppercase tracking-[.1em] text-muted-foreground">Publishers · 30 days</dt><dd className="mt-2 font-serif text-3xl text-navy">{coverageQuery.data.source_coverage.publishers_30d}</dd></div><div className="rounded-xl bg-white p-4 ring-1 ring-border"><dt className="text-[9px] font-bold uppercase tracking-[.1em] text-muted-foreground">Primary/global share</dt><dd className="mt-2 font-serif text-3xl text-navy">{coverageQuery.data.source_coverage.primary_or_global_share_pct.toFixed(1)}%</dd></div><div className="rounded-xl bg-white p-4 ring-1 ring-border"><dt className="text-[9px] font-bold uppercase tracking-[.1em] text-muted-foreground">Leading source share</dt><dd className="mt-2 font-serif text-3xl text-navy">{coverageQuery.data.concentration.leading_source_share_pct.toFixed(1)}%</dd><p className="mt-1 text-[10px] leading-4 text-muted-foreground">of 30-day records</p></div><div className="rounded-xl bg-white p-4 ring-1 ring-border"><dt className="text-[9px] font-bold uppercase tracking-[.1em] text-muted-foreground">Countries without a story</dt><dd className="mt-2 font-serif text-3xl text-navy">{coverageQuery.data.concentration.uncovered_countries_7d}</dd><p className="mt-1 text-[10px] leading-4 text-muted-foreground">of {coverageQuery.data.countries_considered} this week</p></div></dl><p className="mt-4 text-xs leading-5 text-muted-foreground"><strong className="text-navy">Most-covered country:</strong> {countryName(coverageQuery.data.concentration.leading_country_7d.country_code, coverageQuery.data.concentration.leading_country_7d.country_name)} · {coverageQuery.data.concentration.leading_country_7d.records_7d} records · {coverageQuery.data.concentration.leading_country_7d.share_pct.toFixed(1)}% of the seven-day ledger. The four leading sources account for {coverageQuery.data.concentration.top_four_source_share_pct.toFixed(1)}% of the 30-day ledger.</p></div>
                <div><h4 className="text-xs font-bold uppercase tracking-[.12em] text-navy">Direct acquisition health</h4><dl className="mt-3 grid grid-cols-2 gap-2"><div className="rounded-xl border border-border bg-white p-3"><dt className="text-[9px] uppercase tracking-[.08em] text-muted-foreground">Official country lanes</dt><dd className="mt-1 font-serif text-2xl text-navy">{coverageQuery.data.source_network.countries_with_official_lanes} / {coverageQuery.data.countries_considered}</dd></div><div className="rounded-xl border border-border bg-white p-3"><dt className="text-[9px] uppercase tracking-[.08em] text-muted-foreground">Country lanes productive</dt><dd className="mt-1 font-serif text-2xl text-navy">{coverageQuery.data.source_network.official_country_lanes_productive_30d}</dd><p className="text-[9px] text-muted-foreground">last 30 days</p></div><div className="rounded-xl border border-border bg-white p-3"><dt className="text-[9px] uppercase tracking-[.08em] text-muted-foreground">Direct sources productive</dt><dd className="mt-1 font-serif text-2xl text-navy">{coverageQuery.data.source_network.productive_direct_sources_30d} / {coverageQuery.data.source_network.active_direct_sources}</dd></div><div className="rounded-xl border border-border bg-white p-3"><dt className="text-[9px] uppercase tracking-[.08em] text-muted-foreground">Primary/global productive</dt><dd className="mt-1 font-serif text-2xl text-navy">{coverageQuery.data.source_network.productive_primary_or_global_sources_30d} / {coverageQuery.data.source_network.active_primary_or_global_sources}</dd></div></dl><p className="mt-4 text-[10px] leading-4 text-muted-foreground">{coverageQuery.data.source_network.methodology}</p><h4 className="mt-6 text-xs font-bold uppercase tracking-[.12em] text-navy">Leading attributed sources</h4><div className="mt-3 grid gap-2 sm:grid-cols-2">{coverageQuery.data.source_coverage.leading_sources.map(source => <div key={`${source.source_name}-${source.quality_tier}`} className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] gap-3 rounded-xl border border-border bg-white p-3"><div className="min-w-0"><p className="truncate text-xs font-semibold text-navy" title={source.source_name}>{source.source_name}</p><p className="mt-1 text-[9px] uppercase tracking-[.08em] text-muted-foreground">{source.quality_tier === 4 ? 'Primary or global' : source.quality_tier === 3 ? 'Established specialist' : source.quality_tier === 2 ? 'Verified national' : 'Other attributed source'}</p></div><div className="text-right text-[10px] tabular-nums text-muted-foreground"><strong className="block text-sm text-navy">{source.records_30d}</strong>{source.countries_30d} countries</div></div>)}</div><p className="mt-4 text-[10px] leading-4 text-muted-foreground">{coverageQuery.data.source_coverage.methodology}</p></div>
              </div>
            </div>
